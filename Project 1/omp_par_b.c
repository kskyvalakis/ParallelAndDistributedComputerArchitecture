...
void main(int argc, char *argv[])
{
  ...
  int i, j, k, m, n, l, seed, print, nthr, dist=0;
  seed = atoi(argv[1]);
  srand(seed);
  m = atoi(argv[2]);
  n = atoi(argv[3]);
  ...
  for(j=0;j<m;j++){
    //dokimase na svhseis to  collapse(1) reduction(+:dist) na deis poso poly afksanetai o xronos
    #pragma omp parallel for private(i) num_threads(nthr) reduction(+:dist)
    for(k=0;k<n;k++){
      for(i=0;i<l;i++){
